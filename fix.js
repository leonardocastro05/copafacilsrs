...
const pointsToAdd = ["Valdi CT", "André Prov", "Leoo Ferreira", "Eduardo Calvo", "Marc cr Rayo", "Sergio Solano", "Vasvo Lao", "Tony Crowther", "Juan Garcia", "Romus Toras", "Antonio Pinillos", "Freya Anafi", "Julio Maidana", "Thiago Campos", "Francisco Martínez", "Mauro Reyes Hernández"];
pointsToAdd.forEach(p => {
    let rx = new RegExp(`(nombre: .${p}.,[\\s\\S]*?puntos: )(\\d+)`, "g");
    content = content.replace(rx, (m, p1, p2) => {
        let np = parseInt(p2) + 3;
        console.log(p + " gets " + np);
        return p1 + np;
    });
});
...
if (arrayMatch) {
    ...
    while ((match = objRegex.exec(arrayStr)) !== null) {
        pilots.push({ str: match[1], pts: parseInt(match[2]) });
    }
